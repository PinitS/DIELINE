/**
 * Standard NFP (No-Fit Polygon) nesting algorithm — clean, single-pass version.
 *
 * Items are sorted by area descending and placed one at a time using NFP/IFP
 * geometry to find the best bottom-left position across 4 rotations.
 */
...
/**
 * Standard NFP nesting packer (single-pass, area-descending sort).
 *
 * For each item (trying 4 rotations):
 * 1. Compute IFP (inner-fit polygon) with the container.
 * 2. If first item: place at bottom-left corner of IFP.
 * 3. Otherwise: compute NFP with every already-placed polygon,
 *    collect candidate positions from NFP/IFP vertices + edge intersections.
 * 4. Filter: must be inside IFP and outside all NFP interiors.
 * 5. Choose the position with minimum score (y * 1e6 + x) — bottom-left heuristic.
 * 6. Place at the best position across all rotations.
 */
export const nfpStandardPack = (
  items: PreparedModel[],
  usableWidth: number,
  usableHeight: number,
  _layoutDistance: number,
): Placement[] => {
  // Sort items by area descending (single strategy — no multi-pass)
  ...
  for (const prepared of sorted) {
    ...
    for (const rotation of ROTATIONS) {
      ...
      // --- First item: place at bottom-left corner of IFP ---
      if (placedPolygons.length === 0) {
        // IFP is CCW rectangle: [top-left, top-right, bottom-right, bottom-left]
        // Bottom-left corner is ifp[3]
        const pos = ifp[3];
