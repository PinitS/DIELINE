    ...
    return (
      <BaseDielineCanvas
        ...
        renderShape={(layout, shapeStrokeColor, createScenePoint, showShapeLines) => {
          ...
          const { length, width, height, glueWidth, dustFlap, closurePanel } = resolved;
          const {
            frontLeft, frontRight, sideRightRight, backRight, sideLeftRight,
            bodyTopY, bodyBottomY, topY, closureFlapTopY,
          } = guides;
          ...
          const closureDimensionX = pxX(frontLeft + length * 0.34);
          const dustDimensionX = pxX(frontRight + width * 0.56);
          const heightLabelX = Math.max(pxX(backRight) + verticalLabelPadding, rightDimensionX - verticalLabelOffset);
          const closureLabelX = Math.max(pxX(frontLeft) + advancedVerticalLabelPadding, closureDimensionX - advancedVerticalLabelOffset);
          const dustLabelX = Math.max(pxX(frontRight) + advancedVerticalLabelPadding, dustDimensionX - advancedVerticalLabelOffset);
          ...
          return (
            <>
              ...
              {/* Advanced dimension lines: glue, closurePanel, dustFlap */}
              {showDimensions && (
                <>
                  {[
                    [pxX(0), glueDimensionY, pxX(frontLeft), glueDimensionY],
                    [closureDimensionX, pxY(closureFlapTopY), closureDimensionX, pxY(bodyTopY)],
                    [dustDimensionX, pxY(topY), dustDimensionX, pxY(bodyTopY)],
                  ].map(([xS, yS, xE, yE], i) => (
                  ...
                  {[
                    ...
                    [closureDimensionX - advancedDimensionTick / 2, pxY(closureFlapTopY), closureDimensionX + advancedDimensionTick / 2, pxY(closureFlapTopY)],
                    [closureDimensionX - advancedDimensionTick / 2, pxY(bodyTopY), closureDimensionX + advancedDimensionTick / 2, pxY(bodyTopY)],
                    [dustDimensionX - advancedDimensionTick / 2, pxY(topY), dustDimensionX + advancedDimensionTick / 2, pxY(topY)],
                    [dustDimensionX - advancedDimensionTick / 2, pxY(bodyTopY), dustDimensionX + advancedDimensionTick / 2, pxY(bodyTopY)],
                  ].map(([xS, yS, xE, yE], i) => (
              ...
              {/* Advanced dimension text labels */}
              {showDimensions && (
                <>
                  ...
                  <Text position={[closureLabelX, -pxY((closureFlapTopY + bodyTopY) / 2), 3]} color={DIMENSION_COLOR} fontSize={advancedLabelFontSize} anchorX="center" anchorY="middle" textAlign="center" rotation={[0, 0, Math.PI / 2]}>
                    {toLabelText(closurePanel, displayUnit)}
                  </Text>
                  <Text position={[dustLabelX, -pxY((topY + bodyTopY) / 2), 3]} color={DIMENSION_COLOR} fontSize={advancedLabelFontSize} anchorX="center" anchorY="middle" textAlign="center" rotation={[0, 0, Math.PI / 2]}>
                    {toLabelText(dustFlap, displayUnit)}
                  </Text>
                </>
              )}
            </>
