...
const ResponsiveOrthographicCamera = () => {
...
export const BaseDielineCanvas = forwardRef<DielineCanvasHandle, BaseDielineCanvasProps>(
  function BaseDielineCanvas(props, ref) {
    ...
    const onPointerMove = (event: PointerEvent<HTMLDivElement>) => {
      ...
      const deltaX = (event.clientX - dragRef.current.clientX) * (scene.width / rect.width);
      const deltaY = (event.clientY - dragRef.current.clientY) * (scene.height / rect.height);
      dragRef.current = { pointerId: event.pointerId, clientX: event.clientX, clientY: event.clientY };
    ...
    return (
      <div
        ...
        style={{ width, height, overflow: "hidden", borderRadius: 24, background: backgroundColor, boxShadow: "0 12px 24px rgba(0,0,0,0.08)", fontFamily: "inherit", cursor: dragging ? "grabbing" : "grab", touchAction: "none", ...style }}
